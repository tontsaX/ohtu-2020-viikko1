...
public class Varasto {
    ...
    public Varasto(double tilavuus, double alkuSaldo) { // kuormitetaan
    	tarkistaTilavuusArg(tilavuus);
    	tarkistaAlkuSaldoArg(alkuSaldo);
    }
    
    private void tarkistaTilavuusArg(double tilavuus) {
    	if (tilavuus > 0.0) {
            this.tilavuus = tilavuus;
        } else { // virheellinen, nollataan
            this.tilavuus = 0.0;  // => käyttökelvoton varasto
    ...
    private void tarkistaAlkuSaldoArg(double alkuSaldo) {
    	if (alkuSaldo < 0.0) {
            this.saldo = 0.0;
